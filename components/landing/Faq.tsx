...
export default function FAQ() {
    const faqs = [
        ...
        {
            title: "What is the Police Training Application Portal?",
            description: "It is an online platform for applying to accredited police training institutions across Ghana, allowing you to complete your application, payment, and follow-up processes online."
        },
        {
            title: "Who can apply through the portal?",
            description: "All qualified individuals who meet the admission requirements of police training institutions in Ghana."
        },
        {
        ...
        <section className="py-16 bg-white">
            <div className="container">
                ...
                <Accordion type="multiple" className="w-full space-y-4">
                   {faqs.map((faq, index) => ( 
                    <div key={index}>
                        <AccordionItem value={`item-${index}`} className="border border-[#d5e2f6] rounded-lg bg-white print:shadow-none print:border-gray-300">
                        <div className="flex items-center justify-between px-6 py-4 bg-[#e9eff8] hover:bg-[#d5e2f6] rounded-t-lg transition-colors">
                            <AccordionTrigger className="flex-1 text-base font-semibold hover:no-underline p-0 print:text-lg text-[#11386e] hover:text-[#022e6b]">
                                {faq.title}
                            </AccordionTrigger>
                        </div>
